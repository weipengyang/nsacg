...
<body id="cardnews" onload="new Accordian(&#39;basic-accordian&#39;,5,&#39;header_highlight&#39;);" class="mode_webapp">
    ...
    <header>
        <nav id="nav_1" class="p_10">
            <ul class="box">
                <li><a href="index.php?g=Wap&m=Card&a=coupon&token=<?php echo ($token); ?>&wecha_id=<?php echo ($wecha_id); ?>&cardid=<?php echo ($thisCard["id"]); ?>&type=<?php echo ($type); ?>&is_use=0" class="<?php if( $is_use == '0' ): ?>on<?php endif; ?>">未使用</a></li>
                <li><a href="index.php?g=Wap&m=Card&a=coupon&token=<?php echo ($token); ?>&wecha_id=<?php echo ($wecha_id); ?>&cardid=<?php echo ($thisCard["id"]); ?>&type=<?php echo ($type); ?>&is_use=1" class="<?php if( $is_use == '1' ): ?>on<?php endif; ?>">已使用</a></li>
            </ul>
        </nav>
    </header>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><div id="test<?php echo ($item["id"]); ?>-header" class="accordion_headings  <?php if ($item['id']==$firstItemID){?>header_highlight<?php } ?>">
                ...
                <div id="test<?php echo ($item["id"]); ?>-content">
                    <div class="accordion_child">
                        ...
                        <div id="queren<?php echo ($item["id"]); ?>" style="display:none;">
                            <form action="/index.php?g=Wap&m=Card&a=action_useCoupon&wecha_id=<?php echo ($wecha_id); ?>&token=<?php echo ($token); ?>" method="post" id="payform<?php echo ($item["id"]); ?>">
                                <p style=" margin:10px 0 0 0">
                                    <select name="address" class="select" style='width:100px;height:32px' id="address<?php echo ($item["id"]); ?>">
                                        <option value="区府店">区府店</option>
                                        <option value="塘坑店">塘坑店</option>
                                    </select>
                                </p>
                                <p style=" margin:10px 0 0 0">
                                    <input name="password" class="px" id="password<?php echo ($item["id"]); ?>" value="" type="password" placeholder="请输入消费密码">
                                </p>
                                <p style=" margin:10px 0 0 0">
                        ...
                        <?php if($item["is_use"] == '0'): ?><p class="explain_sn" id="shiyong<?php echo ($item["id"]); ?>" style="width: 100%;margin: 0px auto;color: #fff;">
                                <?php if($item["isovertime"] == 1): ?><a style="height: 25px;line-height: 25px;" class="submit tover" href="###" onclick="javascript:void(0);">已经过期了</a>
                                    <?php else: ?>
