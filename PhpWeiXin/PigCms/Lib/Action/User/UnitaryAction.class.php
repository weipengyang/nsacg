...
class UnitaryAction extends UserAction
{
	...
	public function paifa()
	{
		...
		if ($find_lucknum["paifa"] == 1) {
			...
			$model->sendTempMsg("TM00820", array("href" => U("Unitary/goodswhere", array("token" => $this->token, "unitaryid" => $find_lucknum["unitary_id"])), "wecha_id" => $find_lucknum["wecha_id"], "first" => "一元夺宝奖品发货通知", "keynote1" => "已发货", "keynote2" => date("Y年m月d日H时i分s秒"), "remark" => "恭喜您在一元夺宝中获得的【" . $find_unitary["name"] . "】已发货"));
