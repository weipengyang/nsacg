...
//	        }
	...
	public function productSave() {
		...
		$iscoupon = isset($_POST['iscoupon']) ? intval($_POST['iscoupon']) : 0;
		...
		$discount1 = isset($_POST['discount1']) ? htmlspecialchars($_POST['discount1']) : '';
		$discount2 = isset($_POST['discount2']) ? htmlspecialchars($_POST['discount2']) : '';
		$discount3 = isset($_POST['discount3']) ? htmlspecialchars($_POST['discount3']) : '';
		$autosend= isset($_POST['autosend']) ? intval($_POST['autosend']) : 0;
	   ...
	   if (empty($oprice)) {
		...
		$data = array('project'=>$project,'autosend'=>$autosend,'token' => $token, 'gid' => $gid, 'discount1'=>$discount1,'discount2'=>$discount2,'discount3'=>$discount3,'status' => $status,'iscoupon'=>$iscoupon, 'cid' => $this->_cid, 'num' => $num, 'fakemembercount' => $fakemembercount, 'sort' => $sort, 'catid' => $catid, 'name' => $name, 'price' => $price, 'mailprice' => $mailprice, 'vprice' => $vprice, 'oprice' => $oprice, 'intro' => $intro, 'logourl' => $pic, 'keyword' => $keyword, 'time' => time());
