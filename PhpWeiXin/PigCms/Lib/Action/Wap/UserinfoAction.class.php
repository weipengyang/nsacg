	...
	public function index(){
		...
		if(IS_POST){
			...
			if(M('Member_card_custom')->where(array('token'=>$this->token))->getField('tel')){
			 ...
			 $data['carno'] =strtoupper($this->_post('carno'));
			 $data['carno1'] =$this->_post('carno1');
			 $data['carno2'] =$this->_post('carno2');
			      
			 
             $searchkey='%'.$data['carno'].'%';
             ...
             if($count>=2)
             {
                 echo $data['carno'].'车牌已经绑定两个微信，每个车牌最多绑定两个微信';exit;
             }
             if(!empty($data['carno1'])){ 
             $data['carno1']=strtoupper($data['carno1']);
             $searchkey='%'.$data['carno1'].'%';
             $searchwhere['carno']=array('like',$searchkey);
             $searchwhere['carno1']=array('like',$searchkey);
             $searchwhere['carno2']=array('like',$searchkey);
             $searchwhere['_logic']='OR';
             ...
             if($count>=2)
             {
                 echo $data['carno1'].'车牌已经绑定两个微信，每个车牌最多绑定两个微信';exit;
             }
           }
            if(!empty($data['carno2'])){ 
            	$data['carno2']=strtoupper($data['carno2']);
              $searchkey='%'.$data['carno2'].'%';
             $searchwhere['carno']=array('like',$searchkey);
             $searchwhere['carno1']=array('like',$searchkey);
             $searchwhere['carno2']=array('like',$searchkey);
             $searchwhere['_logic']='OR';
             ...
             if($count>=2)
             {
                 echo $data['carno2'].'车牌已经绑定两个微信，每个车牌最多绑定两个微信';exit;
             }
           }
