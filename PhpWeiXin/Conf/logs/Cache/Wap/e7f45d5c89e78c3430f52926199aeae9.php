...
<?php if($memberNotice == NULL): ?><script src="<?php echo RES;?>/css/guajiang/js/wScratchPad.js" type="text/javascript"></script><?php endif; ?>
</head>
...
<?php else: ?>
	<div class="main">
<?php if($Guajiang['usenums'] != 2): if($memberNotice != null){?>
...
$("#save-btn").bind("click",
	function() {
		...
		$.post('index.php?g=Wap&m=Guajiang&a=add&token=<?php echo ($token); ?>', submitData,
			function(data) {
...
function () {
	...
	$.post('index.php?g=Wap&m=Guajiang&a=exchange&token=<?php echo ($token); ?>', submitData,
	function (data) {
		if (data.success == true) {
			alert(data.msg);
