...
class WeixinAction extends BaseAction{
	...
	public function new_pay(){
		...
		if($_GET['from'] == 'consume'){
		    $returnUrl='/index.php?g=Wap&m=Card&a=consumeReturn&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&orderid='.$orderid;
        }
        else
        {
	...
	public function new_return_url (){
		...
		if(intval($_GET['total_fee']) && !intval($_GET['trade_state'])) {
			 Log::record(strtolower($this->from),'DEBUG',true);
			//after
