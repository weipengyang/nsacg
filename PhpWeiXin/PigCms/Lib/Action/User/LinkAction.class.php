...
class LinkAction extends UserAction{
	...
	public function keywordModules(){
		...
		array('module'=>'Baoming','linkcode'=>'','name'=>$this->modules['Baoming'],'sub'=>1,'canselected'=>0,'linkurl'=>'','keyword'=>'','askeyword'=>1),
		);
		//
...
//外链小工具
	...
	public function Vcard(){
		...
		if ($list) {
			foreach ($list as $item) {
				array_push($items, array('id' => $item['id'], 'name' => $item['name'], 'linkcode' => (('{siteUrl}/index.php?g=Wap&m=Baoming&a=index&token=' . $this->token) . '&wecha_id={wechat_id}&id=') . $item['id'], 'linkurl' => '', 'keyword' => $item['keyword']));
			}
		}
		//		
		$this->assign('list', $items);
