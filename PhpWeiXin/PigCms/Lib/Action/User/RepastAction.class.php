...
//		$checkFunc->cfdwdgfds3skgfds3szsd3idsj();
	...
	public function detail()
	{
		...
		if ($orderTable) {
			foreach ($orderTable as $t) {
				$t['tname'] = isset($dinings[$t['tableid']]['name']) ? $dinings[$t['tableid']]['name'] : '';
	...
	public function orders()
	{
		...
		if (IS_POST) {
			...
			$orders = $dish_order->where($newwhere)->order('d_o.id DESC')->limit($Page->firstRow . ',' . $Page->listRows)->field('d_o.*,d_t.dn_id,d_t.reservetime as rrtime')->select();
		}
