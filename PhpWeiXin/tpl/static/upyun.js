...
function memberCardRecharge(uid,cardid){
	...
	art.dialog.open('?g=User&m=Member_card&a=recharge&uid='+uid+'&cardid='+cardid,{id:'changegrade',lock:true,title:'充值会员卡',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.45});
}
function memberCardPresent(uid, cardid,name,carno) {
    art.dialog.data('uid', uid);
    art.dialog.data('cardid', cardid);
    art.dialog.open('?g=User&m=Member_card&a=present&wecha_id=' + uid + '&cardid=' + cardid+ '&truename=' + name+ '&carno=' + carno, {id:'changegrade',lock: true, title: '赠送券', width: 600, height: 400, yesText: '关闭', background: '#000', opacity: 0.45 });
}
