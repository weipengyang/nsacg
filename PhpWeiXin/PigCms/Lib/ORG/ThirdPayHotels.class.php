	...
	public function index($orderid, $paytype = '', $third_id = ''){
		...
		if ($order = M('Hotels_order')->where(array('orderid' => $orderid))->find()) {
			...
			if ($order['paid']) {
				...
				$model->sendTempMsg('TM00820', array('href' => U('Hotels/my',array('token' => $order['token'], 'wecha_id' => $order['wecha_id'])), 'wecha_id' => $order['wecha_id'], 'first' => '预订房间提醒', 'keynote1' => '订单已支付', 'keynote2' => date("Y年m月d日H时i分s秒"), 'remark' => '预订房间成功，感谢您的光临，欢迎下次再次光临！'));
