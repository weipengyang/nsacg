...
//		$checkFunc->cfdwdgfds3skgfds3szsd3idsj();
		if (empty($this->wecha_id) && !in_array(ACTION_NAME, array('gzhurl'))) {
			$this->success('您好，您还没有关注我们的公众号,关注后才能继续喔。', U('Bargain/gzhurl', array('token' => $this->token)));
			exit();
		}
	...
	public function dobuy()
	{
		...
		$this->redirect('Alipay/pay', array('token' => $this->token, 'price' => (int) $order2['bargain_nowprice'], 'wecha_id' => $this->wecha_id, 'from' => 'Bargain', 'orderid' => $order['orderid'], 'single_orderid' => $order['orderid']));
	}
	...
	public function payReturn()
	{
		...
		if ($order['paid'] == 1) {
			...
			$this->error('支付失败', U('Bargain/my', array('token' => $this->token)));
		}
	}
