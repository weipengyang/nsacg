    ...
    public function newpay(){
       if(IS_POST){
           ...
           if($type==1)
           {
               ...
               if(count($couponids)>0){
                   foreach($couponids as $couponid){
                       $r_record=M('Member_card_coupon_record')->where(array('id'=>$couponid))->find();
                       $arr=array(); 
                       $arr['itemid']  	= $itemid;
                       $arr['wecha_id']	= $this->wecha_id;
                       $arr['expense']	= 0;
                       ...
                       $dataArr    = array(
                           ...
                           'url'      => U('Store/cats',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'cardid'=>$r_record['cardid']),true,false,true),
                       );
           ...
           elseif($type==3){
               ...
               $data['当前状态']='结束';
               M('车辆保险','dbo.','difo')->where(array('流水号'=>$itemid))->save($data);
               $this->consumerecord($price,'汽车保险支付',$userinfo,$bx['车牌号码'],$bx['门店']);
               echo '结算成功';
               exit;
           }
           else{
               ...
               M('车辆代办','dbo.','difo')->where(array('流水号'=>$itemid))->save($data);
               $this->consumerecord($price,"代办".$db['代办类别']."支付",$userinfo,$db['车牌号码'],$db['门店']);
               echo '结算成功';
               exit;
           }
           ...
           if($type==1){
               ...
               if($wx['维修类别']=='蜡水洗车'&&strtotime($wx['实际完工'])-strtotime($wx['开工时间'])>60*60&&count($coupon)>0)
               {
                   $tolalprice=0;
                   $isfree=1;
                   ...
                   if($projects){
                       foreach($projects as $project){
                           if(doubleval($project['虚增金额'])>0){
                               ...
                               if($project['券编码']&&doubleval($project['折扣'])>0&&$project['结算方式']!='保险'){
                   ...
                   if($products){
                       foreach($products as $product){
                           if(doubleval($product['虚增金额'])>0){
                               ...
                               if($product['券编码']&&doubleval($product['折扣'])>0&&$product['结算方式']!='保险'){
                                   $where['token']=$this->token;
                                   $where['wecha_id']=$this->wecha_id;
                                   $where['is_use']='0';
                                   $where['over_time']=array('egt',strtotime(date('Y-m-d',time())));
	...
	public function wxcommentSave()
	{
        ...
        if($wx['是否评论']!='是'){
            ...
            if($data['服务态度']<3||$data['服务质量']<3){
                ...
                $model->postMessage($this->getbooturl('总经办'),$msgdata);
                $model->postMessage($this->getbooturl('行政部'),$msgdata);
    ...
    public function washcar(){
        if(IS_POST){
            ...
            if($shop=='')
                $shop=$this->getshopname();
            $carno=$_POST['carno'];
            $unpaycount=M('维修','dbo.','difo')->where(
                array('维修类别'=>'蜡水洗车',
                    '车牌号码'=>$carno,
                    '当前状态'=>array('not in',array('结束','取消'))))->count();
            if($unpaycount>0){
                $this->weixin->send('当前存在未结算的洗车单，结算后才能排队预约。', $this->wecha_id);
                echo '当前存在未结算的洗车单，结算后才能排队预约。';
                return;
            }
            $cancelnum=M('维修','dbo.','difo')->where(
                array('维修类别'=>'蜡水洗车',
                    '车牌号码'=>$carno,
                    '制单日期'=>date('Y-m-d',time()),
                    '当前状态'=>'取消'))->count();
            if($cancelnum>4){
                echo '您今天取消过5次预约，请您到前台排队预约。 ';
                return;
            }
            else {
                $carinfo=$this->genwxrecord('40', $carno, 'AYC10001', '蜡水洗车', $shop);
                ...
                $atime=0;
                if($shop=="区府店"){
                    $atime=intval(floor($wxcount/3))*20;
                }else{
                    $atime=intval(floor($wxcount/2))*20;
                }
                $khlb=$carinfo['客户类别'];
                $message="尊敬的$khlb ,欢迎光临爱车港体验洗车服务，您的前面还有 $wxcount 辆车正在排队，为了减少您的等待时间，请把汽车钥匙交到前台";
                $message.="，预计$atime 分钟后可以开始洗车。洗车时间大约40分钟";
                if (count($coupons) > 0&&$carinfo['客户类别']!='1星客户') {
                    $message.='，超过60分钟，系统自动免单,感谢您的配合。';
                } else {
                    $message.=',感谢您的配合。';
                }
                $this->weixin->send($message, $this->wecha_id);
